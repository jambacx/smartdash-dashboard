...
function Home() {
  const [selectedDate, setSelectedDate] = useState<Date | null>(new Date(new Date().setDate(new Date().getDate() - 14)));
  ...
  return (
    <PageContainer title="Smartdash" description="this is Dashboard">
      <Box>
        <Filter selectedDate={selectedDate} endDate={endDate} setSelectedDate={setSelectedDate} setEndDate={setEndDate} selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory} filterType={filterType} setType={setType} />

        {(listLoading || graphLoading) ? <FallbackSpinner /> :
